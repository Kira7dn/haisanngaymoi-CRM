...
export class S3StorageService {
  ...
  private isValidContentType(
    ...
    const isValid = allowedTypes.some((type) => {
      ...
      return (
        ...
        (type.includes("*") && normalizedContentType.startsWith(typeCategory))
      );
    });

    // If not valid, log for debugging and allow octet-stream as fallback
    if (!isValid) {
      console.warn(
        `[S3StorageService] Content type "${contentType}" not in allowed types for ${fileType}. Allowed:`,
        allowedTypes,
      );
      // Allow octet-stream as fallback for unknown file types
  ...
  async upload(request: FileUploadRequest): Promise<FileUploadResponse> {
    ...
    try {
      // Auto-detect file type if it doesn't match the content type
      let fileType = request.fileType;
      let contentType = request.contentType.toLowerCase();

      // If content type validation fails, try to auto-detect file type
      if (!this.isValidContentType(contentType, fileType)) {
        console.warn(
          `[S3StorageService] Content type "${contentType}" doesn't match fileType "${fileType}". Auto-detecting...`,
        );
        fileType = this.detectFileType(request.fileName, contentType);

        // Try validation again with detected file type
        if (!this.isValidContentType(contentType, fileType)) {
          // If still fails, use octet-stream as fallback
          contentType = "application/octet-stream";
          console.warn(
            `[S3StorageService] Using octet-stream fallback for file: ${request.fileName}`,
          );
        }
      }

      // Generate S3 key with sanitized filename
      const key = this.generateKey(request.fileName, request.folder, fileType);
...
export function createS3StorageService(): S3StorageService {
  ...
  if (
    ...
    console.warn(
      "⚠️ AWS credentials not found. Using mock S3 service in development mode.",
    );
    return {
      upload: async () => ({
        success: true,
  ...
  if (!config.bucket || !config.accessKeyId || !config.secretAccessKey) {
    throw new Error(
      "Missing required AWS S3 configuration. Please set AWS_S3_BUCKET, AWS_ACCESS_KEY_ID, and AWS_SECRET_ACCESS_KEY environment variables.",
    );
  }
